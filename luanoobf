...
local lua_menu = {
    ...
    antiaim = {
        ...
        fakelag_addon = lua_group:checkbox("Fake-Lag Addon"),
...
lua_menu.antiaim.fakelag_addon:depend(aa_tab, aa_main)
...
lua_menu.misc.fix_pvs_animation:depend(misc_tab)
...
local function screen_indicator()
    ...
    local mainx, mainy = renderer.measure_text(lua_menu.visuals.cross_ind_type:get() == 'Default' and "c-" or 'cb', lua_menu.visuals.cross_ind_type:get() == 'Default' and "I G R O K" or 'igrok')
    ...
    if lua_menu.visuals.cross_ind_type:get() == 'Default' then
        ...
        text_fade_animation(center[1] + math.floor(scoped_space * 50), center[2] + 30, -1, {r=r1, g=g1, b=b1, a=255}, {r=r2, g=g2, b=b2, a=255}, "I G R O K", "c-")
        ...
        text_fade_animation(center[1] + math.floor(scoped_space * 52), center[2] + 30, -1, {r=r1, g=g1, b=b1, a=255}, {r=r2, g=g2, b=b2, a=255}, "igrok", "cb")
...
shot_logger.on_aim_hit = function(e)
	...
	shot_logger.add({ info.prefix[1], info.prefix[2], info.prefix[3], 'igrok'}, 
					{ 134, 134, 134, ' » ' }, 
...
shot_logger.on_aim_miss = function(e)
    ...
    shot_logger.add(
        {info.prefix[1], info.prefix[2], info.prefix[3], 'igrok'}, {134, 134, 134, ' » '}, 
...
local function fix_pvs_animation()
    local anim_state = self:get_anim_state()
    if not anim_state then return end

    if anim_state.goal_feet_yaw > 180 or anim_state.goal_feet_yaw < -180 then
        anim_state.goal_feet_yaw = 0
    end

    local overlay = self:get_anim_overlay(0)
    if overlay and (overlay.cycle < 0 or overlay.cycle > 1) then
        overlay.cycle = 0
    end
end
